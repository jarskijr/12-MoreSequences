...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(points)):
        outer_circle = rg.Circle(points[k], 10)
        outer_circle.fill_color = color
        outer_circle.attach_to(window)
    window.render(0.1)
...
def run_test_pizza():
    ...
    # -------------------------------------------------------------------------
    # Test 4:
    title = 'PIZZA test 4:  60 slices, thin (thickness=1) black lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 200)
    circle.outline_thickness = 3
    circle.fill_color = 'yellow'
    pizza(window, circle, 60, 'black', 1)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(points)):
        pizza_line = rg.Line(center, points[k])
        pizza_line.thickness = thickness
        pizza_line.color = color
        pizza_line.attach_to(window)
    window.render(0.1)
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(225, 200), 150)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    circle.attach_to(window)
    for k in range(len(points)):
        first_point = points[k - 1]
        second_point = points[k]
        line = rg.Line(first_point, second_point)
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 4:  300 black lines on blue circle,'
             + ' hops = 61.')
    window = rg.RoseWindow(480, 350, title)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    circle.attach_to(window)
    for k in range(len(points)):
        first_point = points[k % len(points)]
        second_point = points[(k + hops_to_next_point) % len(points)]
        line = rg.Line(first_point, second_point)
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
